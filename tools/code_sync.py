...
@click.option('--dest', default=None, help='Specify a specific destination path on the remote host.')
def code_sync(repo_path, host, include_git, dest):
    ...
    The function also handles excluding the .git directory if the --include-git flag is not set.
    """

    local_path = os.path.abspath(os.path.expanduser(repo_path))
    if not os.path.isdir(local_path):
        click.echo(f'Error: Local path "{local_path}" does not exist or is not a directory.')
        return
    ...
    if dest:
        ...
        if local_path.startswith(home_dir):
            ...
            click.echo('Error: Local path is outside the home directory. Please specify --dest.')
            return
